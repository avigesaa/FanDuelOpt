...
def write_lp_file(player_data, salary_cap):
    """
    ...
    def get_var_name(player):
        return '%s_%s' % (player.position, player.name.replace('-', '').replace('.', '').replace(' ', ''))
    ...
    print 'max: %s;' % ' + '.join(['%s %s' % (p.projection, get_var_name(p)) for p in all_players])
    ...
    print 'sal_lim: %s <= %s;' % (' + '.join(['%s %s' % (p.salary, get_var_name(p)) for p in all_players]), args.salary_cap)
...
if __name__ == '__main__':
    ...
    # Write the .lp file:
    write_lp_file(player_data, args.salary_cap)
